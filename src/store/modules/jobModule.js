...
export default ({
    ...
    getters: {
        ...
        ongoingTasks:(s) => s.jobs.filter(j => j.status === Job.STATUS.ONGOING && j.isTask()),
        pastTasks:(s) =>  s.jobs.filter(j => j.status === (Job.STATUS.FINISHED || j.status === Job.STATUS.CANCELLED) && j.isTask()),
        pastJobs: (s) => s.jobs.filter(j => j.status === (Job.STATUS.FINISHED || j.status === Job.STATUS.CANCELLED) && !j.isTask()),
        ongoingJobs: (s) => s.jobs.filter(j => j.status === Job.STATUS.ONGOING && !j.isTask()).sort((j1, j2) => moment(j1.deadline).format('YYYYMMDD') - moment(j2.deadline).format('YYYYMMDD')),
